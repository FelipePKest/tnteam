...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        local_term_idx_list = [] # track term envs to maintain actor rnn states
        ...
        while True:
            ...
            actions = actions.squeeze(1) # squeeze out the timestep dimension
            cpu_actions = actions.to("cpu").numpy()

            # Update with the actions taken and necessary info
            ...
            pre_transition_data = {
                ...
                "avail_actions": [],
                "obs": [],
                # "actor_hidden_states": actor_hidden_states, # [envs_not_terminated],
            }
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if self.log_discounted_return:
                        episode_returns[idx] += data["reward"] * self.args.gamma**episode_lengths[idx]
                    else:
                    ...
                    if data["terminated"]:
                        ...
                        local_term_idx_list.append(local_term_idx)
        ...
        log_prefix = f"test_" if test_mode else f"train_"
        infos = [cur_stats] + final_env_infos
        cur_stats.update({k: sum(d.get(k, 0) for d in infos) for k in set.union(*[set(d) for d in infos])})
        cur_stats["n_episodes"] = self.batch_size_run + cur_stats.get("n_episodes", 0)
        ...
        n_test_runs = max(1, self.args.test_nepisode // self.batch_size_run) * self.batch_size_run
        ...
        if logged_stats and test_mode and hasattr(self.mac, "get_classifier_accuracy"):
