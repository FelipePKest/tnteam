...
class ModelBasedTeammateEvalAgentLoader(BaseAgentLoader):
    """Load a frozen predictive teammate specialist for decentralized evaluation."""

    preserve_policy_encoder = True

    def __init__(
        self, args, scheme, model_path, load_step, load_agent_idx=0, test_mode=True
    ):
        del load_agent_idx
        ...
        model_state = th.load(
            f"{checkpoint}/teammate_target_model.th", map_location=map_location
        )
        ...
        self.policy.encoder = PredictiveTeammateModel(obs_dim, self.saved_args)
        self.policy.load_state_dict(actor_state)
        self.policy.encoder.load_state_dict(model_state)
        self.policy.eval()
    ...
    def predict(self, ep_batch, agent_idx, t_ep, t_env, bs, test_mode=None):
        ...
        with th.no_grad():
            ...
            actions = self.action_selector.select_action(
                outputs, available, t_env, test_mode=test_mode
            ).reshape(*outputs.shape[:-1], 1)
