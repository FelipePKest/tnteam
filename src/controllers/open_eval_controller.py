...
class OpenEvalMAC:
    ...
    def sample_agent_team(self): 
        ...
        ''' 
        if not self.uncontrolled_type_to_indices:
            uncontrolled_agent_idxs = list(np.random.choice(len(self.uncontrolled_agent_pool),
                                                            self.n_uncontrolled,
                                                            replace=False))
            self.active_uncontrolled_team_idx = None
        else:
            team_idx = np.random.choice(list(self.uncontrolled_type_to_indices.keys()))
            candidates = self.uncontrolled_type_to_indices[team_idx]
            assert len(candidates) >= self.n_uncontrolled, "Not enough agents for selected uncontrolled team"
            uncontrolled_agent_idxs = list(np.random.choice(candidates, self.n_uncontrolled, replace=False))
            self.active_uncontrolled_team_idx = team_idx
    ...
    def _build_agent_pool(self, scheme):
        ...
        for _, agent_cfg in trained_agents_dict.items():
            ...
            if loader_key == "type_conditional_loader":
                ...
                if agent not in self.classifier_agents:
                    self.classifier_agents.append(agent)
                continue
        ...
        self.uncontrolled_agent_types = []
        self.uncontrolled_type_to_indices = {}
        self.uncontrolled_team_name_to_idx = {}
        for team_idx, (team_name, agent_cfg) in enumerate(uncntrl_agents_dict.items()):
            self.uncontrolled_team_name_to_idx[team_name] = team_idx
            self.uncontrolled_type_to_indices[team_idx] = []
            loader_key = agent_cfg["agent_loader"]
            if loader_key == "type_conditional_loader":
                classifier_cfg = agent_cfg.get("classifier", {})
                teammate_cfgs = agent_cfg.get("teammate_types", [])
                ...
                for _ in range(max(1, copies)):
                    ...
                    self.uncontrolled_agent_types.append(team_idx)
                    self.uncontrolled_type_to_indices[team_idx].append(len(self.uncontrolled_agent_pool) - 1)
                continue

            for i in range(agent_cfg["n_agents_to_populate"]):
                if loader_key == "bot_agent_loader":
                    agent = agent_loader_REGISTRY[loader_key](
        ...
        # provide label mapping to any classifier agents
