...
from utils.policy_repr import PolicyRepresentationHead, masked_cross_entropy
...
class PolicyRepresentationPOAMLearner(POAMLearner):
    ...
    def encoder_decoder_update(self, batch, mask, encoder_decoder_train_stats, t_env):
        ...
        for key in ["repr_action_loss", "repr_type_loss", "decoder_loss"]:
        ...
        # `mask` is the ED mask computed by POAMLearner.train. In open training
        # it is already restricted according to ed_model_uncontrolled_only.
        decoder_mask = mask[:, :max_t]
        repr_mask = decoder_mask
        if (
            self.args.open_train_or_eval
            and getattr(self.args, "repr_uncontrolled_only", False)
            and not getattr(self.args, "ed_model_uncontrolled_only", False)
        ):
        ...
        for _ in range(repr_epochs):
            ...
            for indices in sampler:
                ...
                loss = decoder_loss + action_coef * action_loss + type_coef * type_loss
