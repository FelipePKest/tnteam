...
# Training the Uncontrolled-Agent Classifier
Set `learner=classifier_learner` to swap the RL objective for the new transformer classifier that predicts which uncontrolled team was sampled from controlled-agent histories. Example command on Predator-Prey:

```
python src/main.py \
    --config=open/open_train_pp \
    --alg-config=mpe/mapoam \
    --env-config=mpe \
    with learner=classifier_learner env_args.key="mpe:PredatorPrey-v0" seed=112358
```

The classifier automatically consumes the controlled-agent observations plus the trainable-agent masks, and logs `classifier_loss`/`classifier_acc` to Sacred/TensorBoard. Adjust hyperparameters via `with classifier_d_model=256 classifier_lr=5e-4 ...` if needed.
